...
export class RatingStars extends LitElement {
  ...
  async firstUpdated() {
    ...
    if (this.rating > 0 && this.rating <= 5) {
      const el = this.renderRoot.querySelector<HTMLInputElement>("#star-"+Math.round(this.rating));
  ...
  render() {
    return html`
          <div class="ratings ${this.userRating?'user-rating':''} ${this.hasEnoughRatings() || this.userRating?'':'hidden'}">
            ...
            <input @click="${this._handleClick}" class="rating" type="radio" name="stars" id="star-2" value="2"/><label for="star-2"></label>
            <input @click="${this._handleClick}" class="rating" type="radio" name="stars" id="star-1" value="1"/><label for="star-1"></label>
          </div>
